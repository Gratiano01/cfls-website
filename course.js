...
class course {
  ...
  static async read(req, res) {
    ...
    try {
      if (courseId != null) {
        ...
        if (searchCourse != null) returnJson.list.push(searchCourse)
        else returnJson.error_msg = 'Cannot find the course'
      } else if (
        courseName == null &&
        courseType == null &&
        ...
        if (searchCourse != null) {
          for (const Course of searchCourse) returnJson.list.push(Course)
        }
      } else {
        ...
        function selectName(certainJson) {
          ...
          for (const Course of certainJson) {
            if (
              Course.name.indexof(courseName) != -1 ||
              Course.cn_name.indexof(courseName) != -1
            )
              arr.push(Course)
          }
          return arr
        ...
        function selectAttribute(certainJson) {
          var arr = []
          for (const Course of certainJson) {
            if (Course.attribute.includes(courseAttribute)) arr.push(Course)
          }
          return arr
        }
        function selectType(certainJson) {
          var arr = []
          for (const Course of certainJson) {
            if (Course.type.includes(courseType)) arr.push(Course)
          }
          return arr
        }
        if (courseName != null) tempCourse = selectName(tempCourse)
        if (courseType != null) tempCourse = selectType(tempCourse)
        if (courseAttribute != null) tempCourse = selectAttribute(tempCourse)
        if (tempCourse.length == 0)
          returnJson.error_msg = 'Cannot find the course'
        else {
          for (const Course of tempCourse) returnJson.list.push(Course)
        }
      }
      res.status(200).json(returnJson)
    } catch (error) {
      res.status(500).json({ error_msg: '' + error, list: [] })
    }
  }
  ...
  static async smartBox(req, res) {
    try {
      ...
      const searchCourse = await courses.find({})
      if (searchName == null) {
        returnJson.error_msg = 'The parameter (name) is required'
      } else if (searchCourse != null) {
        for (const Course of searchCourse) {
          if (
            Course.name.indexOf(searchName) != -1 ||
            Course.cn_name.indexOf(searchName) != -1
          ) {
            returnJson.list.push(Course.name)
            i++
          }
          if (i > number) break
        }
  ...
  static async create(req, res) {
    try {
      ...
      if (description == null) description = '暂无简介'
      if (teacherId == null) teacherId = [-1]
      if (name == null) {
        returnJson.error_msg = 'The parameter (name) is required'
        returnJson.phase = 'Cannot create a course'
      } else if (cn_name == null) {
        returnJson.error_msg = 'The parameter (cn_name) is required'
        returnJson.phase = 'Cannot create a course'
      } else if (type == null) {
        returnJson.error_msg = 'The parameter (type) is required'
        returnJson.phase = 'Cannot create a course'
      } else if (attribute == null) {
        returnJson.error_msg = 'The parameter (attribute) is required'
        returnJson.phase = 'Cannot create a course'
      } else if (syllabus == null) {
        returnJson.error_msg = 'The parameter (syllabus) is required'
        returnJson.phase = 'Cannot create a course'
      } else if (!Array.isArray(syllabus)) {
        returnJson.error_msg = 'The parameter (syllabus) must be an array'
        returnJson.phase = 'Cannot create a course'
      } else if (!Array.isArray(teacherId)) {
        returnJson.error_msg = 'The parameter (teacherId) must be an array'
        returnJson.phase = 'Cannot create a course'
      } else if (code == null) {
        ...
        var idList = [],
          newId = 0
        const searchCourse = await courses.find({})
        for (const Course of searchCourse) idList.push(Course.id)
        if (idList.length == 0) newId = 0
        else {
        ...
        await newCourses
          ...
          .then((result) => {
            ...
            returnJson.phase =
              'Adding successfully. The new course id is:' + newId
          })
          .catch((error) => {
        ...
        returnJson.phase = 'Cannot create a teacher'
      }
      res.status(200).json(returnJson)
    } catch (error) {
      res
        .status(500)
        .json({ error_msg: '' + error, phase: 'Cannot create the course' })
    }
  }
  ...
  static async update(req, res) {
    try {
      ...
      if (originalId == null && originalName == null) {
        returnJson.error_msg =
          'The parameter (originalName or originalId) is required'
        returnJson.phase = 'Cannot update the course'
            ...
            await courses.findOneAndUpdate(
              { id: originalId },
              {
                $set: {
                  ...
                  cn_name:
                    newCn_name != null ? newCn_name : certainCourse.cn_name,
                  description:
                    newDescription != null
                      ? newDescription
                      : certainCourse.description,
                  type: newType != null ? newType : certainCourse.type,
                  attribute:
                    newAttribute != null
                      ? newAttribute
                      : certainCourse.attribute,
                  syllabus:
                    newSyllabus != null ? newSyllabus : certainCourse.syllabus,
                  teacherId:
                    newTeacherId != null
                      ? newTeacherId
                      : certainCourse.teacherId
                }
              }
            )
          }
                ...
                $set: {
                  ...
                  attribute:
                    newAttribute != null
                      ...
                      : certainCourse.attribute,
                  syllabus:
                    newSyllabus != null ? syllabus : certainCourse.syllabus,
                  teacherId:
                    newTeacherId != null ? teacherId : certainCourse.teacherId
                }
              }
            )
          }
      ...
      res
        .status(500)
        .json({ error_msg: '' + error, phase: 'Cannot update the course' })
    }
  }
