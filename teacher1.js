...
'mongodb+srv://1207888735:qaz1231207@cluster0.1o7e993.mongodb.net/school',
  ...
  const  read = async(req, res) =>{
    try {
      ...
      if (teacherId != null) {
        ...
        if (searchTeacher != null) returnJson.list.push(searchTeacher)
        else returnJson.error_msg = 'Cannot find the teacher'
      } else if (teacherName == null && teacherType == null) {
        //无参数的时输出全部的
        const searchTeacher = await teachers.find({})
        if (searchTeacher != null) {
          for (const Teacher of searchTeacher) returnJson.list.push(Teacher)
        }
      } else {
        ...
        function selectName(certainJson) {
          ...
          for (const Teacher of certainJson) {
            if (Teacher.name.indexOf(teacherName) != -1) arr.push(Teacher)
          }
          return arr
        }
        function selectType(certainJson) {
          var arr = []
          for (const Teacher of certainJson) {
            if (Teacher.type.includes(teacherType)) arr.push(Teacher)
          }
          return arr
        }

        if (teacherName != null) tempTeacher = selectName(tempTeacher)
        if (teacherType != null) tempTeacher = selectType(tempTeacher)

        if (tempTeacher.length == 0)
          returnJson.error_msg = 'Cannot find the teacher'
        else {
          for (const Teacher of tempTeacher) returnJson.list.push(Teacher)
        }
      }
  ...
  const  smartBox = async(req, res) =>{
    try {
      ...
      const searchTeacher = await teachers.find({})
      if (searchName == null) {
        returnJson.error_msg = 'The parameter (searchName) is required'
      } else if (searchTeacher != null) {
        for (const Teacher of searchTeacher) {
          if (Teacher.name.indexOf(searchName) != -1) {
            returnJson.list.push(Teacher.name)
            i++
          }
          if (i > number) break
        }
  ...
  const  create = async(req, res) =>{
    try {
      ...
      if (name == null) {
        ...
        returnJson.error_msg = 'The parameter (type) must be an array'
        returnJson.phase = 'Cannot create a teacher'
      } else if (classesId == null) {
        returnJson.error_msg = 'The parameter (classesId) is required'
        returnJson.phase = 'Cannot create a teacher'
      } else if (!Array.isArray(classesId)) {
        returnJson.error_msg = 'The parameter (classesId) must be an array'
        returnJson.phase = 'Cannot create a teacher'
      } else if (code == null) {
        ...
        var idList = [],
          newId = 0
        const searchTeacher = await teachers.find({})
        for (const Teacher of searchTeacher) idList.push(Teacher.id)
        ...
        await newTeacher
          ...
          .then((result) => {
            ...
            returnJson.phase =
              'Adding successfully. The new teacher id is:' + newId
          })
          .catch((error) => {
      ...
      res
        .status(500)
        .json({ error_msg: '' + error, phase: 'Cannot create the teacher' })
    }
  }
  ...
  const  update = async(req, res) => {
    try {
      ...
      if (originalId == null && originalName == null) {
        ...
        returnJson.error_msg = 'The parameter (newType) must be an Array'
        returnJson.phase = 'Cannot update the teacher'
      } else if (newClassesId != null && !Array.isArray(newClassesId)) {
        returnJson.error_msg = 'The parameter (newClassesId) must be an Array'
        returnJson.phase = 'Cannot update the teacher'
      } else if (code == null) {
        returnJson.error_msg = 'The parameter (code) is required'
        returnJson.phase = 'Cannot update the teacher'
              ...
              { id: originalId },
              {
                $set: {
                  ...
                  avatarsUrl:
                    newAvatarsUrl != null
                      ? newAvatarsUrl
                      : certainTeacher.avatarsUrl,
                  description:
                    newDescription != null
                      ? newDescription
                      : certainTeacher.description,
                  type: newType != null ? newType : certainTeacher.type,
                  classesId:
                    newClassesId != null
                      ? newClassesId
                      : certainTeacher.classesId
                }
              }
            )
            returnJson.error_msg = ''
      ...
      res
        .status(500)
        .json({ error_msg: '' + error, phase: 'Cannot update the teacher' })
    }
  }
...
runAsyncMethods()
module.exports = {
  read,smartBox,create,update,delet
}
