...
const nearlyGoodMessageSchema = new mongoose.Schema({
  name: {
    type: String,
    required: true
  },
...
const NearlyGoodMessage = mongoose.model(
  'NearlyGoodMessage',
  nearlyGoodMessageSchema
)

// Function to create a new nearly good message record
async function createNearlyGoodMessage(awardWinners) {
  try {
...
async function getAllNearlyGoodMessages() {
  try {
